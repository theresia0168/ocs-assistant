...
function makeDie(value, color) {
  ...
  val.className = 'die-val'; val.textContent = value;
...
function rollDice(mode) {
  ...
  if (mode === '1d6') {
    const v = r(); area.appendChild(makeDie(v, 'ivory')); appendTotal(area, v, '합계');
  } else if (mode === '2d6') {
    const [v1,v2] = [r(),r()]; area.appendChild(makeDie(v1,'ivory')); area.appendChild(makeDie(v2,'ivory')); appendTotal(area, v1+v2, '합계');
  } else if (mode === '2d6+1d6') {
    const [v1,v2,v3] = [r(),r(),r()];
    area.appendChild(makeDie(v1,'ivory')); area.appendChild(makeDie(v2,'ivory'));
    const sep = document.createElement('div'); sep.className = 'dice-sep'; sep.textContent = '+'; area.appendChild(sep);
    area.appendChild(makeDie(v3,'red')); appendTotal(area, v1+v2+v3, '합계');
  }
}
...
function rollCombatDice(mode) {
  ...
  if (mode === 'surprise') {
    ...
    let surpriseResult = '', surpriseClass = '';
    ...
    let redResult = '', redClass = '';
    if (surpriseClass === 'atk-surprise') { redResult = `${v3}R`; redClass = 'red-result atk-red'; }
    else if (surpriseClass === 'def-surprise') { redResult = `${v3}L`; redClass = 'red-result def-red'; }
    area.appendChild(makeDie(v1,'ivory')); area.appendChild(makeDie(v2,'ivory'));
    const sep = document.createElement('div'); sep.className = 'dice-sep'; sep.textContent = '+'; area.appendChild(sep);
    ...
    drmDesc += ` = ${finalVal}`;
    modDiv.innerHTML = `<div class="surprise-verdict ${surpriseClass}">${surpriseResult}</div>${redResult ? `<div class="${redClass}">${redResult}</div>` : ''}<div class="drm-line">${drmDesc}</div>`;
    ...
    const mainLog = redResult ? `${surpriseResult}  ${redResult}` : surpriseResult;
    saveToLog('surprise', mainLog, `${typeLabel} 전투 ｜ 주사위 ${drmDesc}`, surpriseClass === 'atk-surprise' ? 'atk-win' : surpriseClass === 'def-surprise' ? 'def-win' : 'no-surp');
    if (surpriseClass === 'atk-surprise')      surpriseState = { type: 'atk', shift: v3 };
    else if (surpriseClass === 'def-surprise') surpriseState = { type: 'def', shift: v3 };
    else                                        surpriseState = { type: 'none', shift: 0 };
    ...
    const [v1,v2] = [r(),r()];
    const rawSum = v1+v2, fortMod = -fortLevel, finalVal = rawSum + arDRM + fortMod;
    area.appendChild(makeDie(v1,'ivory')); area.appendChild(makeDie(v2,'ivory'));
    const modDiv = document.createElement('div');
    modDiv.className = 'dice-modified-total';
    let drmDesc = `${rawSum}`;
    ...
    const typeLabel = combatType === 'overrun' ? '오버런' : '정규';
    saveToLog('combat', result, `${typeLabel} 전투 ｜ ${TERRAIN_LABEL[terrainType]} ${colLabel}${colShift!==0?` (기습 ${colShift>0?'+':''}${colShift})`:''} ｜ 주사위 ${drmDesc}`, 'combat-entry');
  }
}
...
function updateSurpriseIndicator() {
  ...
  if (!surpriseState || surpriseState.type === 'none') {
    indicator.textContent = ''; indicator.className = 'surprise-indicator'; return;
  }
